...
def get_payroll_settings(
    ...
    """ดึงค่าคอนฟิกส่วนกลางทั้งหมดออกมา (All Global Configs)"""
    # รายการคีย์ทั้งหมดที่จำเป็น (Required Keys)
    required_settings = [
    ...
    # ตรวจสอบทีละตัวว่ามีครบหรือยัง (Check each key)
...
def update_payroll_setting(
    ...
    """แอดมินแก้ไขการตั้งค่าส่วนกลาง (Admin Update Global Settings)"""
...
def calculate_user_salary(
    ...
    """
    ฟังก์ชันหลักในการประมวลผลเงินเดือน (Core Calculation Engine)
    ประมวลผลจาก Log การลงเวลา (AttendanceLog) และ โปรไฟล์เงินเดือน (Profile)
    """
    
    # 1. ดึงโปรไฟล์พนักงาน (Get Employee Profile)
    ...
    # 3. ดึง Log การลงเวลาในช่วงเดือนนั้น (Get Attendance Logs)
    # ... (Logic remain same: filtering by date range)
    ...
    # 4. ประมวลผลรายละเอียด (Process Details)
    ...
    # 5. สรุปตัวเลขเงิน (Final Calculation)
    earned_salary = 0
    if salary_type == "monthly":
    ...
    # สูตรคำนวณรายชั่วโมง (Hourly Rate Calculation)
    if salary_type == "monthly":
        # รายเดือน: (Base / Divisor / ShiftHours)
        hourly_rate = (base_salary / std_days_month / std_work_hours)
    else:
        # รายวัน: (DailyWage / ShiftHours)
